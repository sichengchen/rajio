...
public enum RajioCore {
  ...
  public static func parseFeed(feedUrl: String, xml: String, fetchedAt: String) throws -> ParsedFeed
  {
    ...
    let response = try JSONDecoder().decode(
      Response.self, from: Data(String(cString: pointer).utf8))
...
extension RajioCore {
  /// Library policies are evaluated in Rust; the host atomically persists the result.
  public static func library<Result: Decodable>(_ command: [String: Any], as: Result.Type) throws
    -> Result
  {
    let json = String(decoding: try JSONSerialization.data(withJSONObject: command), as: UTF8.self)
    guard let pointer = json.withCString({ rajio_core_library($0) }) else {
      throw CoreError.invalidResponse
    }
    defer { rajio_core_free(pointer) }
    let response = try JSONDecoder().decode(
      Envelope<Result>.self, from: Data(String(cString: pointer).utf8))
    if let error = response.error { throw CoreError.parsing(error) }
    guard let value = response.value else { throw CoreError.invalidResponse }
    return value
  }
