...
export type ControlCommand = { action: ShortcutAction | 'mini' | 'show' | 'settings' | 'seek'; value?: number };
export interface ControlState {
  title: string; show: string; artwork?: string; dark?: boolean; episodeId: string; playing: boolean;
  position: number; duration: number; locale: string; error: string | null;
}
...
export const shortcutLabels = { toggle: 'Play/Pause', back: 'Skip Back', forward: 'Skip Forward', next: 'Next Episode', mini: 'Mini Player' };
export const emptyControlState: ControlState = { title: '', show: '', episodeId: '', playing: false, position: 0, duration: 0, locale: 'en', error: null };
export function validateShortcuts(value: unknown): ShortcutBindings {
  ...
  for (const action of shortcutActions) {
    ...
    if (!item || typeof item.accelerator !== 'string' || typeof item.global !== 'boolean') throw new Error('Invalid shortcut');
    const keys = item.accelerator.split('+');
    const key = keys.pop()!;
    const modifiers = ['CommandOrControl', 'Alt', 'Shift'];
    if (item.accelerator && (!/^([A-Z0-9]|Space|Left|Right|Up|Down|F([1-9]|1[0-9]|2[0-4]))$/.test(key) || keys.some(k => !modifiers.includes(k)) || new Set(keys).size !== keys.length)) throw new Error('Invalid shortcut');
    const accelerator = [...modifiers.filter(k => keys.includes(k)), key].join('+');
    if (item.global && item.accelerator && !keys.includes('CommandOrControl') && !keys.includes('Alt')) throw new Error('Global shortcuts require Command/Ctrl or Alt');
    if (item.accelerator && (used.has(accelerator) || ['CommandOrControl+Q','CommandOrControl+W','CommandOrControl+C','CommandOrControl+V','CommandOrControl+X','CommandOrControl+A','CommandOrControl+Z','CommandOrControl+Shift+Z','CommandOrControl+R'].includes(accelerator))) throw new Error('Shortcut conflict');
