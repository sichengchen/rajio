...
function Player() {
  ...
  useEffect(() => {
    ...
    const stop = api.onState(update);
    void api.get().then(update);
  ...
  const time = (n: number) => {
    ...
    return (
      new Intl.NumberFormat(state.locale, { useGrouping: false }).format(minutes) +
      ":" +
      new Intl.NumberFormat(state.locale, { minimumIntegerDigits: 2, useGrouping: false }).format(
        Math.floor(n % 60),
      )
    );
  };
  return (
    <main
      ...
      data-surface={new URLSearchParams(location.search).get("surface")}
    >
      <header className="compact-titlebar">
        <Button
          variant="ghost"
          size="icon"
          aria-label={t("Open Rajio")}
          title={t("Open Rajio")}
          onClick={() => send("show")}
        >
          <ArrowUpRight />
      ...
      <section className="compact-episode">
        ...
        <div className="min-w-0 flex-1">
          <h1 title={state.title}>{state.title || t("No episode selected")}</h1>
          <p title={state.show}>{state.show}</p>
          <nav aria-label={t("Playback")}>
            <Button
              variant="ghost"
              size="icon"
              aria-label={t("Skip Back")}
      ...
      <div className="compact-progress">
        <Slider
          ...
          step={1}
          value={[seeking ?? state.position]}
          disabled={!state.episodeId || !state.duration}
        ...
        <div className="compact-times">
          <span>{time(seeking ?? state.position)}</span>
          <span>−{time(Math.max(0, state.duration - (seeking ?? state.position)))}</span>
        </div>
      </div>
      {state.error && (
        <p role="alert" className="text-xs text-destructive truncate">
          {state.error}
        </p>
      )}
    </main>
