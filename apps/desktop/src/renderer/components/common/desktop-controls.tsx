...
export function DesktopControls() {
  useEffect(() => {
    ...
    const publish = () => {
      ...
      const p = s.playbackState;
      api.publish({
        dark: document.documentElement.classList.contains("dark"),
        artwork:
          p.currentEpisode?.imageUrl ||
          s.podcasts.find((show) => show.id === p.currentEpisode?.podcastId)?.imageUrl,
        episodeId: p.currentEpisode?.id ?? "",
        title: p.currentEpisode?.title ?? "",
        show: s.podcasts.find((show) => show.id === p.currentEpisode?.podcastId)?.title ?? "",
        playing: p.isPlaying,
    ...
    const unsubscribe = usePodcastStore.subscribe((s, previous) => {
      if (
        s.playbackState === previous.playbackState &&
        s.podcasts === previous.podcasts &&
        s.error === previous.error
      )
        return;
      if (
        s.playbackState.isPlaying !== previous.playbackState.isPlaying ||
        s.playbackState.currentEpisode !== previous.playbackState.currentEpisode
      ) {
        clearTimeout(timer);
        timer = undefined;
        publish();
    ...
    const commands = api.onCommand((cmd) => {
      ...
      if (cmd.action === "back")
        s.seekToTime(Math.max(0, p.currentTime - s.preferences.skipInterval));
      if (cmd.action === "forward")
        s.seekToTime(Math.min(p.duration, p.currentTime + s.preferences.skipInterval));
      if (cmd.action === "next") s.playNextEpisode();
      if (cmd.action === "seek") s.seekToTime(cmd.value!);
    });
    const locale = subscribeLocale(publish);
    const appearance = new MutationObserver(publish);
