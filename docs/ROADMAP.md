...
The shared Rust engine and native/Wasm bindings now run in both production clients. Milestone B delivers independent SwiftUI/GRDB and Electron/SQLite listening, downloads, lifecycle refresh, atomic local outboxes, and seven-language interfaces. The [verification record](milestone-b-verification.md) documents the tested workflows and environments. Milestone C connects these local foundations to multi-user authentication and server-ordered synchronization. See [iOS development](../apps/ios/README.md) and [shared-core build instructions](shared-core.md).
...
B1–B7 are implemented across the SwiftUI application, native Rust engine, GRDB adapter, XcodeGen configuration, and acceptance suites. The following table describes the delivered behavior and verification scope.
...
| B4 — Playback and system integration | Full player with progress and duration, queue, lock-screen controls, background listening, interruption and route recovery | AVPlayer state observation; AVAudioSession intent handling; MediaPlayer metadata/commands; atomic session checkpoints/outbox; restore selected episode paused | Simulator playback and relaunch regressions; signed physical-device build and installation. Physical UI testing is excluded by project direction |
...
The iOS interface follows Apple’s Human Interface Guidelines: native navigation, lists, forms, sheets, semantic typography, adaptive layouts, and Liquid Glass controls. Desktop contributes listening workflows; each platform owns its interface. The installed iPhone build includes the shared Icon Composer icon. See [iOS design conventions](../apps/ios/DESIGN.md) and [verification results](milestone-b-verification.md).
