...
export function rustEnvironment(env = process.env) {
  ...
  if (existsSync(resolve(local, 'cargo/bin/rustup'))) {
    return { ...env, CARGO_HOME: resolve(local, 'cargo'), RUSTUP_HOME: resolve(local, 'rustup'),
      PATH: resolve(local, 'cargo/bin') + delimiter + env.PATH };
  }
  return { ...env, PATH: (env.PATH ?? '') + delimiter + resolve(env.CARGO_HOME ?? resolve(homedir(), '.cargo'), 'bin') };
}
